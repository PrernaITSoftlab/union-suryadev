  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="flex items-center justify-between">
        ...
        <h1 className="text-2xl font-extrabold text-white">Member Accounts Administration</h1>
      </div>

      {/* Filter Bar */}
      <div className="glass-panel rounded-2xl p-4 border border-slate-700/80 max-w-md">
      ...
      {/* Members Data Table */}
      <div className="glass-panel rounded-3xl overflow-hidden border border-slate-700/80 shadow-card-dark">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-800 text-slate-300">
              {loading ? (
                <tr><td colSpan="5" className="py-12 text-center text-slate-500">Loading member database...</td></tr>
                ...
                <tr key={m.id} className="hover:bg-slate-800/40 transition-colors">
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div>
                        <div className="font-bold text-white">{m.profile?.full_name || 'User'}</div>
                  ...
                  <td className="py-4 px-6">
                    <button
                      onClick={() => handleUpdateStatus(m.id, null, m.role === 'admin' ? 'member' : 'admin')}
                      className={`px-2.5 py-1 rounded-md text-[10px] font-extrabold uppercase ${
                        m.role === 'admin' ? 'bg-amber-500/20 text-amber-300 border border-amber-500/40' : 'bg-slate-800 text-slate-300'
                      }`}
                    >
                      {m.role}
                    </button>
                  ...
                  <td className="py-4 px-6">
                    <div className="font-semibold text-slate-200">{m.profile?.company || 'N/A'}</div>
                    <div className="text-[11px] text-slate-400">{m.profile?.industry || 'N/A'}</div>
                  </td>
                  ...
                  <td className="py-4 px-6 text-right space-x-2">
                    {m.status === 'active' ? (
                      <button
                        ...
                        className="px-3 py-1 rounded-lg bg-rose-500/10 text-rose-400 text-[11px] font-bold hover:bg-rose-500/20"
                      >
                        Suspend
                      </button>
                    ) : (
                      <button
                        ...
                        className="px-3 py-1 rounded-lg bg-emerald-500/10 text-emerald-400 text-[11px] font-bold hover:bg-emerald-500/20"
                      >
                        Activate
                      </button>
                    )}
